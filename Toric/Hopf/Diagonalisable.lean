...
lemma span_isGroupLikeElem_eq_top_of_isDiagonalisable :
  ...
  rintro ⟨⟨G, _, ⟨e⟩⟩⟩
  rw [(Set.setOf_inj.mpr (funext (fun (a : A) ↦ propext_iff.mpr
    (isGroupLikeElem_map (a := a) (BialgEquivClass.toBialgEquiv e))))).symm.trans
    (Equiv.setOf_apply_symm_eq_image_setOf (BialgEquivClass.toBialgEquiv e).toEquiv.symm _),
    ← LinearEquiv.range (BialgEquivClass.toBialgEquiv e).toLinearEquiv.symm, ← Submodule.map_top,
    ← MonoidAlgebra.span_isGroupLikeElem (R := R) (A := R) (G := G), Submodule.map_span]
  rfl
