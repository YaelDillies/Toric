...
section CommRing
variable {R A : Type*} [CommRing R] [IsDomain R] [Ring A] [Bialgebra R A] [NoZeroSMulDivisors R A]

open MonoidAlgebra

/-- A bialgebra over a domain that is generated by its group-like elements is isomorphic
...
lemma liftGroupLikeBialgHom_bijective_of_span_isGroupLikeElem_eq_top
  ...
  set e : GroupLike R A ≃ {a : A | IsGroupLikeElem R a} := {
    toFun g := ⟨g.1, g.2⟩
...
/-- A bialgebra over a domain that is generated by its group-like elements is diagonalisable.
...
lemma IsDiagonalisable.of_span_isGroupLikeElem_eq_top
  ...
  existsIso := ⟨GroupLike R A, inferInstance, ⟨.symm <| BialgEquiv.ofBijective <|
    liftGroupLikeBialgHom_bijective_of_span_isGroupLikeElem_eq_top h⟩⟩

/-- A bialgebra over a domain is diagonalizable if and only if it is generated by its group-like
elements.
