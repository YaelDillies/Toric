...
noncomputable def commMonAlg : CommMonCatᵒᵖ ⥤ Mon_ (CommAlgCat R)ᵒᵖ where
  obj M := .mk' <| op <| CommAlgCat.of R <| MonoidAlgebra R M.unop.carrier
  map f := .mk' (CommAlgCat.ofHom <| MonoidAlgebra.mapDomainBialgHom R f.unop.hom).op
  map_comp f g := by ext; simp

/-- The functor of commutative group algebras. -/
noncomputable def commGrpAlg : CommGrpᵒᵖ ⥤ Grp_ (CommAlgCat R)ᵒᵖ where
  obj G := .mk' <| op <| CommAlgCat.of R <| MonoidAlgebra R G.unop.carrier
  map f := Grp_.homMk (CommAlgCat.ofHom <| MonoidAlgebra.mapDomainBialgHom R f.unop.hom).op
  map_comp f g := by ext; simp
