...
variable {R : Type u} {B : Type v} [CommRing R] [Semiring B] [Bialgebra R B]
...
class IsDiagonalisable : Prop where
  existsIso :
    ∃ (A : Type w) (_ : Group A), Nonempty (B ≃ₐc[R] MonoidAlgebra R A)

/--
A group algebra is diagonalisable.
-/
instance {A : Type w} [Group A] : IsDiagonalisable.{w} R (MonoidAlgebra R A) :=
...
lemma isDiagonalisable_iff_isDiagonalisable_sections : IsDiagonalisable G ↔ IsAffine G.left ∧
    HopfAlgebra.IsDiagonalisable K (Γ.obj <| op G.left) := by
  refine ⟨fun ⟨⟨A, _, ⟨e⟩⟩⟩ ↦ ⟨?_, ?_⟩,
    fun ⟨aff, diag⟩ ↦ {existsIso := ⟨GroupLike K (Γ.obj <| op G.left), sorry, Nonempty.intro ?_⟩}⟩
  · sorry
  · sorry
  · sorry
...
lemma isDiagonalisable_iff_span_isGroupLikeElem_eq_top :
  ...
  rw [isDiagonalisable_iff_isDiagonalisable_sections,
