...
abbrev char := Grp_.mk' (asOver G S) ⟶ .mk' <| asOver 𝔾ₘ[S] S

variable (S G) in
/-- The cocharacters of the group scheme `G` over `S` are the group morphisms `𝔾ₘ[S] ⟶/S G`. -/
abbrev cochar := Grp_.mk' (asOver 𝔾ₘ[S] S) ⟶ .mk' (asOver G S)
...
noncomputable def charPairing : X*(S, G) →* X(S, G) →* X(S, 𝔾ₘ[S]) where
  toFun χ := ((CommGrp_.yonedaCommGrpGrp.obj (.mk' <| asOver 𝔾ₘ[S] S)).map χ.op).hom
